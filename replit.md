...
- `LessonView` keyboard shortcuts: Space/Enter/Right-arrow advances, A/B/C/D picks a checkpoint answer. Logic is inlined inside the keydown effect (not delegated to `next()`/`pickAnswer()`) so the effect only depends on real state and never sees stale closures. A `confettiTimer` ref tracks the auto-hide timeout so it's cancelled on step change and on unmount, preventing a previous step's celebration from flickering off mid-frame on the next one.
- Bilingual narration: `web/src/components/Narration.tsx` is a small Web Speech API button (Listen/Stop, with SVG speaker / pause icons) used inside intro and concept steps. It cancels any in-flight utterance when the text or language changes (and on unmount), uses `es-US`/`en-US` voices, and renders nothing when the browser lacks `speechSynthesis`. Veo videos are silent so this gives each step real voice-over.
- Home dashboard shows a "▶ N video lessons" pill (count comes straight from `video-manifest.json`) so users see how much real video content is part of the experience.
